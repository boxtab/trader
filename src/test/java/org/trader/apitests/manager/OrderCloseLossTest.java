...
public class OrderCloseLossTest
{
    ...
    void testCloseLossOrders() throws Exception
    {
        ...
        System.out.println("Отправляем запрос: GET /manager/order/close-loss/" + accountId);

        var response = given()
                .contentType(ContentType.JSON)
                .header("Authorization", "Bearer " + token)
                .when()
                .get("/manager/order/close-loss/" + accountId)
                .then()
                .statusCode(200)
                .extract()
                .response();
        ...
        System.out.println("Закрыто убыточных ордеров: " + closedCount);

        // Проверки
        assertThat(status).isEqualTo("ok");
        assertThat(closedCount).isNotNull().isGreaterThanOrEqualTo(0);

        System.out.println("Тест успешно завершен! Закрыто убыточных ордеров: " + closedCount);
    }
}
